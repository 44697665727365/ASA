...
public:
	...
	{
		return FreeCRT::Math::Sqrt((X * X) + (Y * Y) + (Z * Z));
	}
	...
	{
		return FreeCRT::Math::IsFinite(this->X) && FreeCRT::Math::IsFinite(this->Y) && FreeCRT::Math::IsFinite(this->Z);
	}
};
...
public:
	...
	UnderlayingType Magnitude() const
	{
		return FreeCRT::Math::Sqrt((X * X) + (Y * Y));
	}
...
public:
	...
	static float RGBChannelToLinear(uint8_t channel)
	{
		float sRGB = channel / 255.0f;

		// Apply the sRGB to linear conversion curve
		if (sRGB <= 0.04045f)
		{
			return sRGB / 12.92f;  // Linearized for values below a certain threshold
		}
		else
		{
			return FreeCRT::Math::PowF((sRGB + 0.055f) / 1.055f, 2.4f);  // Higher values follow a different curve
		}
	}

	static FLinearColor FromRGB(int R, int G, int B)
	{
		return FLinearColor(
			RGBChannelToLinear(R),
			RGBChannelToLinear(G),
			RGBChannelToLinear(B),
			1.0f  // Full opacity by default
		);
	}

	static FLinearColor FromARGB(int A, int R, int G, int B)
	{
		return FLinearColor(
			RGBChannelToLinear(R),
			RGBChannelToLinear(G),
			RGBChannelToLinear(B),
			FreeCRT::TypeTraits::Clamp((float)A / 255.0f, 0.0f, 1.0f)
		);
	}
};
